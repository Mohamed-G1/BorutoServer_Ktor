...
class HeroRepositoryImpl : HeroRepository {
    ...
    override suspend fun getAllHeroes(page: Int): ApiResponse {
        return ApiResponse(
            success = true,
            message = "Success",
            prevPage = calculatePages(page = page)[PREVIOUS_PAGE_KEY],
            nextPage = calculatePages(page = page)[NEXT_PAGE_KEY],
            heroes = heroes[page]!!
        )
    }
...
//    private fun calculatePages(page: Int) =
//        mapOf(
//            PREVIOUS_PAGE_KEY to if (page in 2..5) page.minus(1) else null,
//            NEXT_PAGE_KEY to if (page in 1..4) page.plus(1) else null
//        )

    override suspend fun searchOnHeroes(name: String?): ApiResponse {
        return ApiResponse(
            success = true,
            message = "Success",
            heroes = foundHeroes(query = name)
        )
    }

    private fun foundHeroes(query: String?): List<Hero> {
        val foundedHeroes = mutableListOf<Hero>()
        return if (!query.isNullOrEmpty()) {
            heroes.forEach { (i, heroes) ->
                heroes.forEach { hero ->
